...
class Model(nn.Module):
    def __init__(
        ...
        for out_dim in hidden_dims:
            ...
            self.dropouts.append(nn.Dropout(dropoutt))
            in_dim = out_dim
...
layer_dims_list = [[16, 32], [16, 32, 64], [16, 32, 64, 128]]
dropouts = [0.2]
...
for layer_dims in layer_dims_list:
    for dropout in dropouts:
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(graphs)):
            ...
            model = Model(
                hidden_dims=layer_dims, edge_attr_dim=1, dropoutt=dropout, output_dim=4
            ).to(device)
