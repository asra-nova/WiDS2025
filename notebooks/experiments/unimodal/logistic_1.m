%% 1. Load All Data Files with Original Column Names
train_cat = readtable('TRAIN_CATEGORICAL_METADATA_new.xlsx', 'VariableNamingRule', 'preserve');
train_quant = readtable('TRAIN_QUANTITATIVE_METADATA_new.xlsx', 'VariableNamingRule', 'preserve');
train_conn = readtable('TRAIN_FUNCTIONAL_CONNECTOME_MATRICES_new_36P_Pearson.csv', 'VariableNamingRule', 'preserve');
test_cat = readtable('TEST_CATEGORICAL.xlsx', 'VariableNamingRule', 'preserve');
test_quant = readtable('TEST_QUANTITATIVE_METADATA.xlsx', 'VariableNamingRule', 'preserve');
test_conn = readtable('TEST_FUNCTIONAL_CONNECTOME_MATRICES.csv', 'VariableNamingRule', 'preserve');
train_solutions = readtable('TRAINING_SOLUTIONS.xlsx', 'VariableNamingRule', 'preserve');

%% 2. Match Participants Across All Training Tables
[common_ids_cat_quant, idx_cat, idx_quant] = intersect(train_cat.participant_id, train_quant.participant_id, 'stable');
[common_ids_cat_quant_conn, idx_cat_quant, idx_conn] = intersect(common_ids_cat_quant, train_conn.participant_id, 'stable');
[common_train_ids, idx_common, idx_solutions] = intersect(common_ids_cat_quant_conn, train_solutions.participant_id, 'stable');

% Update indices to map to original tables
idx_cat = idx_cat(idx_cat_quant(idx_common));
idx_quant = idx_quant(idx_cat_quant(idx_common));
idx_conn = idx_conn(idx_common);

% Filter training tables to only include common participants
train_cat = train_cat(idx_cat, :);
train_quant = train_quant(idx_quant, :);
train_conn = train_conn(idx_conn, :);
train_solutions = train_solutions(idx_solutions, :);

%% 3. Create Dummy Variables for Categorical Data
cat_vars = setdiff(train_cat.Properties.VariableNames, 'participant_id', 'stable');
train_dummies = table(common_train_ids, 'VariableNames', {'participant_id'});
test_dummies = table(test_quant.participant_id, 'VariableNames', {'participant_id'});

for i = 1:length(cat_vars)
    current_var = cat_vars{i};
    train_cat_values = string(train_cat.(current_var));
    test_cat_values = string(test_cat.(current_var));
    cats = unique(train_cat_values);
    cats = cats(~ismissing(cats));
    
    for j = 1:length(cats)
        col_name = matlab.lang.makeValidName([current_var '_' char(cats(j))]);
        train_dummies.(col_name) = double(strcmp(train_cat_values, cats(j)));
        test_dummies.(col_name) = double(strcmp(test_cat_values, cats(j)));
    end
end

%% 4. Process Quantitative Data
quant_vars = setdiff(train_quant.Properties.VariableNames, 'participant_id', 'stable');

train_quant_clean = train_quant(:, quant_vars);
for i = 1:width(train_quant_clean)
    col_data = train_quant_clean.(i);
    col_data(isnan(col_data)) = median(col_data, 'omitnan');
    train_quant_clean.(i) = col_data;
end

test_quant_clean = test_quant(:, quant_vars);
for i = 1:width(test_quant_clean)
    col_data = test_quant_clean.(i);
    col_data(isnan(col_data)) = median(train_quant_clean.(i), 'omitnan');
    test_quant_clean.(i) = col_data;
end

%% 5. Process Connectome Data
...
X_train_full = [table2array(train_quant_clean), table2array(train_dummies(:, 2:end)), table2array(train_conn_feat)];
X_test = [table2array(test_quant_clean), table2array(test_dummies(:, 2:end)), table2array(test_conn_feat)];

%% 7. Load Target Variables from TRAINING_SOLUTIONS
y_adhd_full = train_solutions.ADHD_Outcome; % Actual ADHD target
y_sex_full = train_solutions.Sex_F;         % Actual Sex target (1 = Female, 0 = Not Female)

%% 8. Split Training Data into Train and Validation Sets
rng(42);
...
%% 9. Feature Selection for ADHD and Sex
...
%% 10. Compute Class Weights for Imbalance
...
%% 11. Train ADHD Model with Logistic Regression and Hyperparameter Tuning
...
%% 12. Train Sex Model with Logistic Regression and Hyperparameter Tuning
...
%% 13. Validate Models and Compute Weighted F1 Scores
...
submission_logistic = table(test_quant.participant_id, test_adhd_pred, test_sex_pred, ...
